...
VAD::VAD(Codec codec, int audioSampleRate, int channels) {
    ...
    this->DecodedData = std::shared_ptr<opus_int16>(new opus_int16[150000]);
    if (codec == Codec::Opus) {
...
void VAD::StartWorker() {
    //this->DecodedData = new opus_int16[15000];

    //this->Data = new unsigned char[3000];
    int error = -69;
   // opus_int16 DecodedData[150000];
    //int DecodedDataLen = 0;

    this->vadModule = WebRtcVad_Create();
    ...
    this->Decoder = opus_decoder_create(SampleRate, Channels, &error);
...
void VAD::DecodeVoice() {
    ...
    int frame_size = opus_decode(Decoder, Data, DataLen, DecodedData.get(), sample * Channels, 0);
    //std::cout << "Decoded" << std::endl;
    DecodedDataLen = frame_size * Channels * sizeof(opus_int16);

    if (frame_size > 0) {
        //WebRtcVad_ValidRateAndFrameLength(int rate, size_t frame_length)
        int voiceDetected = WebRtcVad_Process(vadModule, SampleRate, DecodedData.get(), frame_size);
...
//    SSL_CTX_set_min_proto_version(ctx, DTLS1_2_VERSION);
    ...
    //    ERR_print_errors_fp(stderr);
	   // exit(0);
    //}
    if (SSL_CTX_use_certificate(ctx, Certificate::cert) <= 0) {
        ERR_print_errors_fp(stderr);
        exit(0);
    }

    if (SSL_CTX_use_PrivateKey(ctx, Certificate::key) <= 0) {
        ERR_print_errors_fp(stderr);
        exit(0);
    }
...
//
//                }
//                std::unique_lock<std::mutex> wlock(packetsLock);
//                //packets.pop_front();
...
void RTPHandler::ForwardPackets(unsigned char* packetdata, int packetdataLen, bool rtcp)
{
    ...
    if (rtcp) {
        ...
        int err = srtp_protect_rtcp(srtpSession2, packetdata, &packetdataLen);
        if (err == 0) {
            sendto(sock, (const char*) packetdata, packetdataLen, MSG_DONTWAIT, (struct sockaddr*)&addr, len);
        ...
        int err = srtp_protect(srtpSession2, packetdata, &packetdataLen);
        if (err == 0) {
            //std::cout << "Sending data"  << err << std::endl;
            sendto(sock, (const char*) packetdata, packetdataLen, MSG_DONTWAIT, (struct sockaddr*)&addr, len);
...
void RTPHandler::Listen(std::string address, int  port)
{
    ...
    //     notifyLock.unlock();
        ...
        sock = create_socket("0.0.0.0", port);
                        ...
                        if (SenderMode) {
                            ...
                            if (err == 0) {
                                ...
                                if (isAudioStream) {
                                    ...
                                    vad = std::shared_ptr<VAD>(DBG_NEW VAD(audioCodec, audioSampleRate, channels));
                                }
                            }
                            else {
                                //std::cout << "SRTP Session couldn't be created. " << err << std::endl;
 ...
 //                       while (true) {
                ...
                if (isSrtpSetup) {
                        ...
                        if (buffer[1] == 200 || buffer[1] == 201 || buffer[1] == 204 || buffer[1] == 205) {
                           ...
                           // std::async(std::launch::async, &RTPHandler::SignalReaders, this, buffer, packetLen, 1);
          ...
          /*                  _packetLen = packetLen;
                            ...
                            typ = 0;
                            waitForDataFromSource.notify_one();*/
                            //std::async(std::launch::async, &RTPHandler::SignalReaders, this, buffer, packetLen, 0);
    ...
    //while (true) {
        ...
        if (err == 0) {
            ...
            if (SenderMode) {
                ...
                source->ForwardPackets(dataToSend, _packetLen, typ);
                //RTPPacket packet{};
                //packet.id = 0;
                //if (typ == 1) {
                //    packet.rtcp = true;
                ...
                //    std::unique_lock<std::mutex> wlock(source->packetsLock);
            ...
            //RTPPacket packet{};
            ////packet.data = std::shared_ptr<unsigned char>(new unsigned char[_packetLen]);
            //packet.id = 0;
            //memcpy(&packet.data, dataToSend, _packetLen);
            //packet.packetLen = _packetLen;
            //if (typ == 1) {
            //    packet.rtcp = true;
            //}
            unsigned char* readerData = new unsigned char[3000];
            int readerDataLen = 0;
            ...
            for (auto reader : readers) {
                ...
                if (reader != nullptr) {
                    ...
                    readerDataLen = _packetLen;
                    memcpy(readerData, dataToSend, _packetLen);
                    reader->ForwardPackets(readerData, readerDataLen, typ);

                    //std::unique_lock<std::mutex> wlock(reader->packetsLock);
                    //if (reader->packets.size() > 8) {
            ...
            if ( isAudioStream) {
                ...
                memcpy(vad->Data, dataToSend + 28, (uint32_t) _packetLen -  28);
...
RTPHandler::~RTPHandler() {
    ...
    std::cout << "IJUST WANNNNAAAAAAAAAAAAAA SHUT DOWN ~!!!!!!!!!!!!!!!!!!@#!@#@!#@!#@!#\n" << std::endl;
    Shutdown();
